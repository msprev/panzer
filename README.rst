...
3. What about the new lua filters introduced in pandoc 2.0? These
   ...
   dedicated style fields) would actually defeat their purpose by making
   panzer’s processing slower. The motivations for using the new lua
   filters are (i) speed; (ii) avoiding external dependencies. Neither
   of these are typical concerns in a use case of panzer. Anything that
   can be achieved with a lua filter (in terms of document manipulation)
   can easily be achieved with a traditional JSON filter. I recommend
   that unless (i) or (ii) are your priority (in which case you probably
   shouldn’t be using panzer), you should use a JSON filter.
