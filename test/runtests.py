...
-   Does not test binary pandoc writers (docx, odt, etc.) or pdf output
    -   Problem: these can't be cleanly diff'ed
...
def parse_cli(argv):
    ...
    if len(argv) < 2 or argv[1] not in possible_remits:
        ...
        print('     where REMIT could be "pandoc" or "panzer" or "diff"')
        print('     if no sources specified, then all sources are tested')
        sys.exit(1)
    if len(argv) >= 2:
        remit = argv[1]
    if len(argv) > 2:
