...
This documentation is a work in progress. 
    ...
    panzer is pretty much complete.
    Current focus is to:
...
Style definitions are hierarchically structured by *style name* and *writer*.
    ...
    Style names by convention are MixedCase (`Notes`).
...
Here are some [example style definitions](http://https://github.com/msprev/dot-panzer).
    ...
    Nevertheless, they should give an idea of how easy it is to write useful style definitions and related executables.
...
Inheritance among style settings follows only four rules.
    Fields (including runlists) specified in the document metadata override any style setting.
...
There are some intuitive wrinkles regarding what 'overrides' means for different style properties.
    Generally, runlists are *additive* while other fields are *non-additive*.
...
For runlists specified by `preflight`, `filter`, `postflight` and `cleanup` the union is non-destructive.
    An overriding definition simply adds its runlist items after higher precedence items.

This creates a puzzle about how to remove an items from a runlist.
    This is accomplished by issuing a specific command.
...
Filters specified on the command line (as a value of `--filter`) are run first: they are the first items in the runlist.
...
When panzer is searching for an executable or template, say filter `foo`, it will search in the following places from first to last
...
panzer works ok with existing pandoc filters.
    But not all filters that will work with panzer will work with pandoc.
