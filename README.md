...
should probably be using a filter).
  ...
  - `RUNLIST` is a list of processes in the run list; it has the
    ...
    'command': 'my command', 'arguments': ['argument1', 'argument2',
  ...
  - `OPTIONS` is a dictionary containing panzer’s and pandoc’s command
    ...
    const.DEFAULT_SUPPORT_DIR, 'pandoc': 'pandoc', 'debug': str(),
    'quiet': False, 'strict': False, 'stdin_temp_file': str() # tempfile
    used to buffer stdin }, 'pandoc': { 'input': list(), # list of input
    files 'output': '-', # output file; '-' is stdout 'pdf_output':
    False, # if pandoc will write a .pdf 'read': str(), # reader
    'write': str(), # writer 'options': {'r': dict(), 'w': dict()} } }`
